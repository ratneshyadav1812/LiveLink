...
const socketHandlerV1 = (socket) => {
    ...
    socket.on(socketActions_1.ACTIONS.MUTE, ({ roomId, userId }) => {
        ...
        socket.broadcast.to(roomId).emit(socketActions_1.ACTIONS.MUTE, {
            peerId: socket.id,
            userId
        });
    });
    socket.on(socketActions_1.ACTIONS.UNMUTE, ({ roomId, userId }) => {
        // Broadcast to everyone ELSE in the room
        socket.broadcast.to(roomId).emit(socketActions_1.ACTIONS.UNMUTE, {
            peerId: socket.id,
            userId
        });
    });
};
exports.socketHandlerV1 = socketHandlerV1;
